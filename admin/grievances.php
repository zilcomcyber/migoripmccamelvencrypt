...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajax'])) {
    ...
    switch ($action) {
        case 'respond_grievance':
            ...
            // Get grievance details and verify ownership with decryption
            $grievance_data = EncryptionHelper::selectDecrypted($pdo, "
                SELECT f.*, p.created_by, p.project_name 
                FROM feedback f
                JOIN projects p ON f.project_id = p.id
                WHERE f.id = ? AND f.status = 'grievance' AND p.created_by = ?
            ", [$feedback_id, $current_admin['id']], 'feedback', false);
            
            $grievance = $grievance_data ?: null;
            ...
            try {
                // Update grievance with response using encrypted update
                $result = EncryptionHelper::updateEncrypted($pdo, 'feedback', [
                ...
                if (!empty($grievance['citizen_email'])) {
                    ...
                    if (!$email_sent) {
                        error_log("Failed to send email notification for grievance #$feedback_id to {$grievance['citizen_email']}");
                        // Don't fail the entire operation if email fails
                    }
                } else {
                    error_log("No email address found for grievance #$feedback_id - cannot send notification");
                }
        ...
        case 'update_status':
            ...
            // Verify grievance exists and admin owns the project with decryption
            $grievance_data = EncryptionHelper::selectDecrypted($pdo, "
...
// Get grievances for current admin with proper decryption
$grievances = EncryptionHelper::selectDecrypted($pdo, "
