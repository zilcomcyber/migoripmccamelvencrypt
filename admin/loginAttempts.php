...
// Modified code to ensure proper encryption handling for login attempts display.
require_once 'includes/pageSecurity.php';
require_once __DIR__ . '/../config.php';
require_once __DIR__ . '/../includes/auth.php';
require_once __DIR__ . '/../includes/functions.php';
require_once __DIR__ . '/../includes/dataInterceptor.php';
...
try {
    ...
    // If we have login attempts, ensure they are properly decrypted for display
    if (!empty($login_attempts) && SystemSettings::isEncryptionEnabled()) {
        foreach ($login_attempts as &$attempt) {
            // Check if email appears to be encrypted and decrypt if needed
            if (isset($attempt['email']) && strpos($attempt['email'], '@') === false && strlen($attempt['email']) > 20) {
                try {
                    $decrypted_email = DataEncryption::decrypt($attempt['email']);
                    if (filter_var($decrypted_email, FILTER_VALIDATE_EMAIL)) {
                        $attempt['email'] = $decrypted_email;
                    }
                } catch (Exception $e) {
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm text-gray-500 max-w-xs truncate" title="<?php echo htmlspecialchars($attempt['user_agent'] ?? 'Unknown'); ?>">
                                        ...
                                        echo htmlspecialchars(truncate_text($user_agent, 50));
                                        ?>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                ...
                <div class="px-6 py-4 border-t border-gray-200">
                    <div class="flex items-center justify-between">
                        <div class="text-sm text-gray-700">
                            Showing <?php echo (($filters['page'] - 1) * $per_page) + 1; ?> to 
                            <?php echo min($filters['page'] * $per_page, $total_attempts); ?> of 
