...
class SystemSettings {
    ...
    private static function initializeSetting($key, $default_value, $description = '', $type = 'string') {
        try {
            $stmt = self::$pdo->prepare("SELECT setting_value FROM system_settings WHERE setting_key = ?");
            $stmt->execute([$key]);
    ...
    public static function get($key, $default = null) {
        ...
        try {
            $stmt = self::$pdo->prepare("SELECT setting_value, setting_type FROM system_settings WHERE setting_key = ?");
            $stmt->execute([$key]);
            $result = $stmt->fetch();
    ...
    public static function isEncryptionEnabled() {
        // Fallback if SystemSettings is not properly initialized
        if (!self::$pdo) {
            return defined('ENCRYPTION_ENABLED') ? ENCRYPTION_ENABLED : true;
        }
