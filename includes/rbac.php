...
class SecureRBAC {
    ...
    private static $page_permissions = [
        ...
        'systemSettings.php' => 'super_admin_only',
    ...
    private static $permission_categories = [
        ...
        'System Administration' => [
            ...
            'super_admin_only' => [
                ...
                'description' => 'Access restricted to super administrators only',
                'risk_level' => 'critical'
            ],
            'manage_encryption' => [
        ...
        'Financial Management' => [
            'financial_management' => [
                ...
                'description' => 'Access comprehensive financial ledger system',
                'risk_level' => 'high'
            ],
            ...
            'cost_management' => [
                ...
                'description' => 'Manage WBS and CBS structures',
                'risk_level' => 'medium'
            ]
        ]
    ];
    ...
    private static function createRequiredTables() {
        try {
            ...
            if ($stmt->rowCount() == 0) {
             ...
             $stmt = self::$pdo->query("SHOW TABLES LIKE 'transactions'");
             if ($stmt->rowCount() == 0) {
                 self::$pdo->exec("
             ...
             $stmt = self::$pdo->query("SHOW TABLES LIKE 'purchase_requests'");
             if ($stmt->rowCount() == 0) {
                 self::$pdo->exec("
             ...
             $stmt = self::$pdo->query("SHOW TABLES LIKE 'purchase_orders'");
             if ($stmt->rowCount() == 0) {
                 self::$pdo->exec("
             ...
             $stmt = self::$pdo->query("SHOW TABLES LIKE 'wbs'");
             if ($stmt->rowCount() == 0) {
                 self::$pdo->exec("
                     CREATE TABLE wbs (
                         id INT AUTO_INCREMENT PRIMARY KEY,
             ...
             // Check if cbs table exists
             $stmt = self::$pdo->query("SHOW TABLES LIKE 'cbs'");
             if ($stmt->rowCount() == 0) {
                 self::$pdo->exec("
                     CREATE TABLE cbs (
                         id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    public static function checkPagePermission($page_filename) {
        ...
        // Special handling for super admin only pages
        if ($required_permission === 'super_admin_only') {
            if ($current_admin['role'] !== 'super_admin') {
                self::logSecurityEvent('super_admin_only_access_attempt', $current_admin['id'], [
                    'page' => $page_filename,
...
function requirePagePermission($page_filename) {
    return SecureRBAC::requirePagePermission($page_filename);
}
