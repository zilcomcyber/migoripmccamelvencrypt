    ...
    try {
        ...
        if (EncryptionManager::getEncryptionMode()) {
            ...
            foreach ($attempts as $attempt) {
                $decrypted_attempt = EncryptionManager::processDataForReading('login_attempts', $attempt);
                if (strtolower($decrypted_attempt['email']) === strtolower($email)) {
                    $failed_count++;
                }
            }
...
function record_login_attempt($email, $status = 'fail', $user_id = null, $failure_reason = null) {
    // Use the standardized log_login_attempt function from functions.php
 ...
 * @param string $status The status of the login attempt ('success' or 'fail').
 * @param int|null $user_id The ID of the user, if available.
 * @param string|null $failure_reason The reason for failure, if applicable.
 */
