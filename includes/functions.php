...
/**
 * Safe PDO operations with basic audit logging
 */
...
function pdo_insert($pdo, $table, $data) {
    try {
        ...
        if ($result && table_needs_audit_trail($table)) {
            ...
            if ($admin_id) {
                AuditTrail::logActivity($admin_id, 'create', $table, $record_id, "Created new record in {$table}");
            }
        }
    ...
    try {
        ...
        if ($result && table_needs_audit_trail($table) && $original_data) {
            ...
            if ($admin_id) {
                ...
                AuditTrail::logActivity($admin_id, 'update', $table, $record_id, "Updated record in {$table}", [
...
function require_project_ownership($project_id, $permission_key = null) {
    ...
    if (!owns_project($project_id, $current_admin['id'], $pdo)) {
        ...
        if (function_exists('AuditTrail::logActivity')) {
            AuditTrail::logActivity($current_admin['id'], 'unauthorized_access', 'projects', $project_id, 'Unauthorized project access attempt');
        }
...
function edit_transaction_with_history($transaction_id, $new_data, $edit_reason, $admin_id) {
    ...
    try {
        ...
        // Get the original transaction with decryption
 ...
 */
function log_activity($activity_type, $description, $admin_id = null, $category = 'system', $related_id = null, $context = []) {
    global $pdo;
    ...
    try {
        // Use DataInterceptor to handle encryption based on current mode
        $data = [
            'admin_id' => $admin_id,
        ...
        $encrypted_data = EncryptionManager::processDataForStorage('admin_activity_log', $data);

        $sql = "INSERT INTO activity_logs (admin_id, activity_type, description, category, related_id, context, ip_address, user_agent, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";

        $stmt = $pdo->prepare($sql);
        $stmt->execute([
            $encrypted_data['admin_id'],
            $encrypted_data['activity_type'],
            $encrypted_data['description'],
            $encrypted_data['category'],
            $encrypted_data['related_id'],
            $encrypted_data['context'],
            $encrypted_data['ip_address'],
            $encrypted_data['user_agent'],
            $encrypted_data['created_at']
        ]);
    } catch (PDOException $e) {
        error_log("Logging activity failed: " . $e->getMessage());
        // Suppress error to prevent application crash, but consider more robust error handling in production
    }
}
...
class CacheManager {
    ...
    public static function init() {
        if (self::$cache_dir === null) {
            self::$cache_dir = sys_get_temp_dir() . '/project_cache/';
    ...
    public static function get($key) {
        ...
        if (file_exists($cache_file)) {
            ...
            if ($cache_data['expires'] > time()) {
                self::$cache[$key] = $cache_data['data'];
 ...
 */
function get_login_attempts($filters = [], $paginate = false, $per_page = 20) {
    global $pdo;

    $sql = "SELECT la.*, a.name as user_name 
            FROM login_attempts la 
            LEFT JOIN admins a ON la.user_id = a.id 
            WHERE 1=1";

    $count_sql = "SELECT COUNT(*) as total FROM login_attempts WHERE 1=1";
    $params = $count_params = [];
    ...
    if (!empty($filters['email'])) {
        ...
        $count_params[] = $filters['email'];
    }

    // IP address filter
    if (!empty($filters['ip_address'])) {
        $sql .= " AND la.ip_address = ?";
        $count_sql .= " AND ip_address = ?";
        $params[] = $filters['ip_address'];
        $count_params[] = $filters['ip_address'];
    }
    ...
    $sql .= " ORDER BY la.timestamp DESC";
    ...
    // Use pdo_select for proper encryption handling
    $attempts = pdo_select($pdo, $sql, $params, 'login_attempts', true);
 ...
 */
function get_login_attempt_stats($days = 7) {
    global $pdo;
    ...
    try {
        ...
        // Get top failed IPs
        $failed_ips_sql = "SELECT ip_address, COUNT(*) as attempt_count
                          FROM login_attempts 
                          WHERE status = 'fail' 
        ...
        $stmt = $pdo->prepare($failed_ips_sql);
        $stmt->execute([$days]);
        $failed_ips = $stmt->fetchAll();
...
function get_fund_sources() {
    ...
    $sql = "SELECT * FROM fund_sources ORDER BY source_name ASC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll();
}
...
function get_prepared_responses() {
    ...
    $sql = "SELECT * FROM prepared_responses ORDER BY name ASC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll();
}
 ...
 */
function send_notification_email($to, $subject, $message, $is_html = true) {
    ...
    $headers = [
        'From: ' . (SITE_EMAIL ?? 'noreply@migoricounty.go.ke'),
        'Reply-To: ' . (SITE_EMAIL ?? 'noreply@migoricounty.go.ke'),
        'X-Mailer: PHP/' . phpversion()
    ];
...
/**
 * Project progress recalculation using proper 50/50 calculation
 */
function recalculate_all_project_progress() {
    global $pdo;
    ...
    try {
        ...
        foreach ($projects as $project_id) {
            // Use the proper progress calculation that implements 50% steps + 50% financial
            $result = update_project_progress_and_status($project_id, false, true);
            if ($result['success']) {
                $updated_count++;
            }
        }

        log_activity('bulk_progress_recalculation', "Recalculated progress for {$updated_count} projects", null, 'system');
        return ['success' => true, 'updated_count' => $updated_count];
        
    } catch (Exception $e) {
        error_log("Recalculate all project progress error: " . $e->getMessage());
        return ['success' => false, 'message' => $e->getMessage()];
    }
}
...
/**
 * Function to validate and sanitize file upload data
 */
function validate_and_sanitize_file($file) {
    ...
    // Sanitize file type
    $file['type'] = filter_var($file['type'], FILTER_SANITIZE_STRING);

    // Sanitize file size
    $file['size'] = filter_var($file['size'], FILTER_SANITIZE_NUMBER_INT);

    // Sanitize temporary file name
    $file['tmp_name'] = filter_var($file['tmp_name'], FILTER_SANITIZE_STRING);

    // Return the sanitized file data
    return $file;
}

/**
 * Function to get all system roles
 */
function get_system_roles() {
    ...
    try {
        ...
        error_log("Get system roles error: " . $e->getMessage());
        return [];
    }
}
