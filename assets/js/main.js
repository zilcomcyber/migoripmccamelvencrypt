        ...
        commentForm.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (data.success) {
                    // Determine notification type based on message content
                    let notificationType = 'success';
                    if (data.message.toLowerCase().includes('review') || data.message.toLowerCase().includes('pending')) {
                        notificationType = 'pending';
                    } else if (data.message.toLowerCase().includes('language') || data.message.toLowerCase().includes('blocked')) {
                        notificationType = 'blocked';
                    } else if (data.message.toLowerCase().includes('approved') || data.message.toLowerCase().includes('successfully')) {
                ...
                // Show error notification
                showEnhancedNotification('Network error: Please check your connection and try again', 'error');

                if (responseMessage) {
                    responseMessage.classList.remove('hidden', 'bg-blue-100', 'bg-green-100', 'text-blue-800', 'text-green-800', 'border-blue-200', 'border-green-200');
                    responseMessage.classList.add('bg-red-100', 'text-red-800', 'border-red-200');
                    responseMessage.innerHTML = '<i class="fas fa-exclamation-triangle mr-2"></i>Network error: Please check your connection and try again';
